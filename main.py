...
re_acts = r'<h2>(.*?)</h2>'
acts_list = re.findall(re_acts, content)  # funktioniert
...
stageDirections_list = re.findall(re_stageDirections, content)  # funktioniert

re_texts = r'</b>(.*?)<br>'
texts_list = re.findall(re_texts, content)   # keine Ausgabe

re_scenes = r'\[(.*?)\]'
scenes_list = re.findall(re_scenes, content)  # funktioniert
